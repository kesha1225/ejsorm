...
from ._types import EJData, EJTable, EJTableData, EJObject
from .exceptions import EJError
from .filters import EJ_FILTERS
from .models import EJModel, EJ_OBJECT_ID_FIELD
...
class Ejsorm:
    ...
    def _loads_data(self):
        ...
        with open(self.path) as file:
            ...
            if data[0].get("__ejsorm__") != "generated":
                raise EJError(
                    "File is not generated by ejsorm, clear it or delete or use db.drop() and rerun"
                )
        return data

    def __save(self, data: EJData):
        if self._data is None:
            self._data = data
    ...
    def commit(self):
        # todo если данных новых нет че нить делать надо
        self._data[0]["__temp__"] = []
        self.__save(self._data)

    def rollback(self):
        # todo если данных новых нет че нить делать надо
        self._data[1:] = self._loads_data()[1:]
        self._data[0]["__temp__"] = []
        self.__save(self._data)

    def _get_table(self, table_name: str) -> EJTable:
    ...
    def _get_table_data(self, table_name: str) -> EJTableData:
        current_table = self._get_table(table_name)
        current_table_data: EJTableData = current_table["__data__"]
        return current_table_data

    def _get_table_data_from_obj(
        self, find_obj: typing.Union[typing.Type[EJModel], str]
    ):
    ...
    def _save_element(self, table_name: str, obj: EJObject) -> int:
        ...
        # тк эта штука берется из self.data, при добавлении объекта сюда меняется и self.data
        ...
        for obj_key, obj_value in obj.items():
            for find_key, find_value in check_kwargs.items():
                if (
                    ...
                    and find_key != EJ_OBJECT_ID_FIELD
                ...
                if find_key == obj_key:
                    ...
                    if isinstance(obj_value, dict) and isinstance(find_value, dict):
                        if obj_value.get(EJ_OBJECT_ID_FIELD) is not None:
    ...
    def find_one(
        self,
        find_obj: typing.Union[typing.Type[EJModel], str],
        return_raw: bool = False,
        with_refs: bool = True,
        **kwargs,
    ) -> typing.Optional[typing.Union[EJObject, typing.Any]]:
        ...
        if issubclass(find_obj, EJModel):
            references = self._get_object_references_fields(find_obj.__fields__)
            find_obj.__fields__[EJ_OBJECT_ID_FIELD] = PK_FIELD
            for ref in references:
                ref.type_.__fields__[EJ_OBJECT_ID_FIELD] = PK_FIELD
    ...
    def find_all(
        ...
        with_refs: bool = True,
        **kwargs,
    ) -> typing.Optional[typing.Union[typing.List[typing.Union[EJObject, typing.Any]]]]:
        self._check_return_raw_and_get_refs(return_raw, with_refs)
        ...
        if return_raw:
            return found_objs
        return [find_obj(**obj) for obj in found_objs if issubclass(find_obj, EJModel)]
    ...
    def is_reference(ref_data_to_find):
        return isinstance(ref_data_to_find, str) and ref_data_to_find.startswith(
            "__reference"
        )

    def _add_to_table(self, table_name: str, obj: EJObject, obj_class: EJModel):
    ...
    def delete(self, table_name: str, to_delete: EJObject):
        table_data = self._get_table_data(table_name)
        if to_delete not in table_data:
            raise EJError("No such element in db")
        self._get_table_data(table_name).remove(to_delete)

    def update(self, obj: EJModel, model_id: int) -> EJModel:
        old = self.find_one(
            obj.__class__.__name__,
            **{EJ_OBJECT_ID_FIELD: model_id},
            return_raw=True,
            with_refs=False,
        )
    ...
    def write_many(self, objects: typing.List[EJModel]):
